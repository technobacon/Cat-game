...
func test_place_emits_room_changed() -> void:
	...
	var before: int = RoomState.placement_count()
...
func test_placing_and_storing_is_free() -> void:
	var coins_before: int = EconomyManager.coins()
	RoomState.place(&"rug", Vector2i(0, 0))
	var pid: int = RoomState.placements()[0].pid
	RoomState.store(pid)
...
func test_dump_restore_round_trip() -> void:
	...
	var snapshot: Dictionary = RoomState.dump()
